...
# keep each "Table N" caption on the same page as its table
body = re.sub(r"(<p><strong>Table \d+[^<]*</strong>[^<]*</p>)\s*<table>",
              r"<div style='page-break-inside: avoid'>\1<table>", body)
n = body.count("<div style='page-break-inside: avoid'>")
parts = body.split("</table>")
for i in range(len(parts) - 1):
    if n <= 0: break
    if "<div style='page-break-inside: avoid'>" in parts[i]:
        parts[i] = parts[i]; parts[i+1] = "</div>" + parts[i+1] if not parts[i+1].startswith("</div>") else parts[i+1]
        n -= 1
body = "</table>".join(parts)
