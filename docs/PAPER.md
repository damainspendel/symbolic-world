...
Large language models extract knowledge-graph triples from text at 30–66% fact accuracy, and deployed citation systems hallucinate 11–57% of citations — rates that make naive LLM extraction unusable for scholarship. **The contribution of this paper is a methodology, not a dataset**: an approach to AI-assisted knowledge extraction in which trust is manufactured by construction, measured at every stage, and terminates — by design — in human judgment; the knowledge graph we build is the working demonstration. We present a pipeline in which **no assertion can enter the published artifact without passing (i) a deterministic verbatim-quotation check against its cited source paragraph and (ii) an independent full-paragraph review by a model from a different family than the proposer**, and we apply it to build The Symbolic World: a knowledge graph of C. G. Jung's *Collected Works* comprising **231 nodes, 627 edges, and 652 references** (100% verification coverage, with machine-readable verdict provenance per citation). Each reference is anchored to a numbered CW paragraph with a ≤25-word verbatim quote and typed by rhetorical voice (*jung-asserts* / *jung-reports-parallel* / *jung-quotes-source*), separating Jung's own claims from doctrine he reports — a first-class treatment of a failure mode we find dominant in practice. We evaluate the pipeline itself: retro-verification of early unggated output measures a **~25% flaw rate in unverified extraction**; a blind seeded-corruption calibration (n=40) measures the verifier at ceiling on structural corruptions (reversals 5/5, object swaps 5/5, specificity 20/20) with the deficit concentrated in attribution-voice flips (2/5) — per-class samples are small (n=5) and rates carry wide intervals, so we report raw fractions throughout; an adversarial audit of 30 random published references finds **0 content errors** and 13.3% modality refinements; a full-graph modality sweep (n=634) corrects 10.3% of references on the voice/hedge axis, with the flag rate falling from 18.9% (oldest extractions) to 5.6% (newest) — evidence that verifier feedback compounds into extractor precision (three consecutive 20/20 batches). A cross-vendor audit (Gemini 3.1 Pro, blind, n=100) finds 0 unsupported edges under the shared rubric with 11 nuance-level disagreements (4 upheld and corrected); a follow-up **rubric-free** audit of the full CW 14 volume (n=407), in the second vendor's own terms, upholds a **1.0% residual error rate** (4 corrections, none fabrications or reversals) and shows the shared rubric was not steering verdicts (identical detection profile with and without it). The specialist-narrowing effect replicates cross-vendor (voice-flip detection 2/6→4/6 vs first-party 2/5→5/6). Marginal cost is ≈ **$0.09–0.12 per fully verified citation**, orders of magnitude below expert manual construction. All error rates, seeds, adjudications, and a 16-item assumptions register are published with the artifact, alongside a standing public falsification offer. The trust chain deliberately does not terminate in models: every edge carries a check-it-yourself pointer, and the artifact ships live human channels — per-edge **dispute** and **confirmation** reports with a published adjudication log — so that reader verification accumulates as first-class provenance. The machine gates are the floor of the method; humans are its ceiling.
...
5. **Verification decomposes along semantic risk axes.** A verifier asked to judge five criteria at once distributes attention by salience: structural axes reach ceiling while the subtle axis (voice) drops to 40% detection. The identical model with a single-axis brief reaches 5/6 (E5; small n — the effect's strength is its cross-vendor replication, not any single run). The practical architecture is therefore a structure generalist plus one or two specialists on the corpus's *measured* weak axes — not an agent per criterion (specialists pay only where the generalist is weak, and cross-axis errors still need a whole-paragraph reading). The principle generalizes: each corpus has its own risk axes — rhetorical voice for Jung; holding-vs-dictum for law; claim-vs-contemporary-belief for history of science.
