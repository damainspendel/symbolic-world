...
**What we borrowed.** The reference format adapts the assertion/provenance/publication-info anatomy of [nanopublications](https://arxiv.org/pdf/1809.06532) (a native serialization is planned), and the voice-typing treats provenance as *epistemic stance* in the sense of [provenance-enhanced statements](https://arxiv.org/html/2606.15246) — our stubborn "sympathetic reportage" residual is precisely their claim-in-permeation between cognitive worlds. The nearest existing artifact is the [Darshana Graph](https://arxiv.org/abs/2606.18222), whose author names a randomly sampled precision evaluation "the most valuable immediate extension" of that work — this paper operationalizes exactly that extension, with the caveat that our annotators are so far models from two vendors rather than multiple humans (human tier live as a channel, §8). No knowledge graph of Jung's Collected Works previously existed; the ARAS concordance and Princeton's digital edition are search, not relations.
...
1. *Propose* (Claude Opus 4.8): 12–22 candidate edges from a contiguous window; verbatim quotes; node-vocabulary reuse; claim-typing rules in the generic vocabulary *author-asserts / author-reports / author-quotes* (reported doctrine must be typed as reported; the case study instantiates these as `jung-asserts` etc.).
2. *Mechanical pre-check* (deterministic): each quote must appear in its cited paragraph as a letter-normalized, in-order, **near-contiguous** token sequence — gaps of at most 3 tokens between consecutive quote words, absorbing the corpus's interleaved figure/footnote markers while rejecting collage quotes assembled from scattered words (a dedicated canary guards this property); explicit ellipses split a quote into fragments checked independently; plus node existence and duplicate-triple rejection. The publication gate is also structural: the atlas build refuses to run if any reference lacks `verified: true`.
...
Every mitigant gets measured, by a small set of reusable experiment designs: retro-verification of ungated output, seeded-corruption calibration of each gate, adversarial audit of the published artifact, independent re-extraction, and cross-vendor audit in two modes (shared rubric and rubric-free). None of these designs is corpus-specific. Figure 4 pins each experiment to the step it tests; full details are in Appendix A, and the consolidated results are Table 1 (§10).
