...
| Dimension | Typical existing work | This methodology |
|---|---|---|
| What gets verified | sampled or selected outputs (PhilKG; spot checks) | every published claim, as a publication gate |
| Verifier independence | same model, or a stronger model from the same ecosystem | different model family at the gate; different **vendor** at the audit |
| Is the verifier itself measured? | rarely | seeded-corruption calibration with published catch rates and CIs |
| Evidence granularity | citation or document level | verbatim quote anchored to a numbered paragraph, mechanically checked |
| Whose claim is it? | untyped | voice-typed (asserts / reports / quotes), hedges demoted |
| Error reporting | aggregate accuracy | per-risk residuals, published adjudications, public dispute log |
