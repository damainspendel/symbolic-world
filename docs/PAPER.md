...
- **Cross-vendor audit, two modes** (measures R6, and the audit instrument itself). A different vendor's model re-judges published samples, once under our rubric (comparable) and once entirely in its own terms (independent): shared blind spots, and whether the rubric steers verdicts.
...
**Instantiation.** The generic voice vocabulary becomes `jung-asserts` / `jung-reports-parallel` / `jung-quotes-source` (+ named source). Current state: **241 nodes · 648 edges · 673 references** across 479 distinct paragraphs of nine CW volumes; CW 14 covered end-to-end; 450 asserts / 162 reports / 61 quotes-source over 74 named sources; 85 hedged (medium-confidence) references; 100% gate-verified with per-reference verifier and date.
...
|---|---|---|---|---|
| R1 | **Wrong claims** | structure gate (E1b-calibrated) | E2: 0/30 adversarial; E6/E7: 0 unsupported | near zero post-gate |
...
| R3 | **Wrong meaning** | structure gate | reversal/object-swap caught 92–96% (E1b) | near zero post-gate |
| R4 | **Wrong voice** | voice gate + graph-wide sweep | detector catches 70–80%; sweep fixed 10.3% | **~2.6% estimated — the dominant surviving risk; no current audit can bound it** |
...
1. **Ensure extraction is verified: the checking is the product.** A quarter of unverified extractions were flawed; after gating, adversarial audits find zero structural errors (direction, referent, fabrication) — the voice residual is Imperative 2's subject.
...
The compounded result, stated with matched accounting at both ends. At the *flag* level (anything a reviewer questioned, before adjudication): raw extraction was flagged at 25.2% (E0), the published graph at 12.5% (E7) — a twofold reduction. At the *adjudicated* level: 4 of the second vendor's flags were upheld on the classes that vendor can independently detect (1.0%), and on the voice class — which both vendors demonstrably miss in the same places, so no current audit can bound it — our own sweep data implies a further residual of roughly 2.6% (E3's 10.3% flag rate over E5b's 80% detector sensitivity). The honest summary: fabrication is eliminated; structural error is reduced to near zero; the surviving risk is attribution nuance at a few percent, estimated rather than bounded; the marginal API cost is about ten cents per verified claim.
